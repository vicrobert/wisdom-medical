...
public class HospitalServiceImpl implements HospitalService {
	...
	public List<HospitalVo> listAllHospitals(Pageable pageable) {
		...
		hospVoIter.forEach(e->{respList.add(e);});
		return respList;
	}
	...
	public DepartmentVo getDepartment(Long departmentId) {
		return departmentDao.findById(departmentId).orElseGet(()->{return null;});
	}
	 ...
	 * @param treeLevel
	 * 		if treeLevel less&equal -1 then pull all departments with different level
	 */
	@Override
	...
	public DoctorVo getDoctorById(Long doctorId) {
		return doctorDao.findById(doctorId).orElseGet(()->{return null;});
	}
